...
def negSamplingLossAndGradient(
    ...
    second_term = np.dot(U_k.T, 1-z)
    gradCenterVec += second_term
    gradOutsideVecs[negSampleWordIndices] += np.outer((1-z), centerWordVec)

    #Counter the case where same index occurs multiple times
    indexCount = np.bincount(indices)
    for idx in np.unique(indices):
        gradOutsideVecs[idx] *= indexCount[idx]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentWordVec = centerWordVectors[centerIdx]
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        (l, gradCentreVec, gradOutsideVector) = word2vecLossAndGradient(currentWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += l
        gradCenterVecs[centerIdx] += gradCentreVec
